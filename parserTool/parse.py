...
Language.build_library(
    ...
    os.path.join(project_path, "PASMG/parserTool", "my-languages.so"),

    # Include one or more languages
    [
        os.path.join(project_path, "PASMG/parserTool", "tree-sitter-c"),
        os.path.join(project_path, "PASMG/parserTool", "tree-sitter-java")
    ]
)
...
def java():
    ...
    parser.set_language(Language(os.path.join(
        project_path, "PASMG/parserTool", "my-languages.so"), "java"))
    return parser

def python():
    parser = Parser()
    parser.set_language(Language(os.path.join(
        project_path, "PASMG/parserTool", "my-languages.so"), "python"))
    return parser

def c():
    parser = Parser()
    parser.set_language(Language(os.path.join(
        project_path, "PASMG/parserTool", "my-languages.so"), "c"))
    return parser
